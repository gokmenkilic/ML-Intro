...
contains
  subroutine pair_gpu(d_x, d_y, d_z, d_g, natoms, nframes, xbox, ybox, zbox, box, del)
    ...
    do iconf = 1, nframes
       ! TODO : Use the right parallel construct below
       ! Note: The parallel construct combines both do loops
       do ?? (i=1:natoms, j=1:natoms)
          ...
          if (r < cut) then
             ind = int(r / del) + 1
             !Note: We are using OpenACC atomic to ensure atomic operations on GPU
             !$acc atomic
             d_g(ind) = d_g(ind) + 1
          end if
       end do
