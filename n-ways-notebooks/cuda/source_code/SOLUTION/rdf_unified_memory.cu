...
int main(int argc, char *argv[]) {
  ...
  cudaMallocManaged(&h_x, sizef);
  cudaMallocManaged(&h_y, sizef);
  cudaMallocManaged(&h_z, sizef);
  cudaMallocManaged(&h_g, sizebin);
  ...
  pair_gpu<<<nblock, nthreads>>>(h_x, h_y, h_z, h_g, numatm, nconf, xbox, ybox,
                                 zbox, box, del);
  nvtxRangePop();
