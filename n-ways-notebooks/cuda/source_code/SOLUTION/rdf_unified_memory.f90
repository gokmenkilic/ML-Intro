...
program rdf
  ...
  ! Note: Using the "managed" keyword to indicate managed Unified memory
  real(4), managed, allocatable :: h_x(:)
  real(4), managed, allocatable :: h_y(:)
  real(4), managed, allocatable :: h_z(:)
  integer, managed, allocatable :: h_g(:)
  ...
  ! Note: Subroutine is called with managed variables!
  call pair_gpu <<<nblock, nthreads>>> (h_x, h_y, h_z, h_g, natoms, &
