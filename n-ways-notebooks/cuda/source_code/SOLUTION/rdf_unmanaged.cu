...
__global__ void pair_gpu(const float *d_x, const float *d_y, const float *d_z,
...
int main(int argc, char *argv[]) {
  ...
  device = 0;
  cudaSetDevice(device); // Note: pick the device to use
  //////////////////////////////////////////////////////////////////////////////
  ...
  cudaMalloc((void **)&d_g, sizebin);

  // Note: Copying from input arrays from Host to Device
  cudaMemcpy(d_g, h_g, sizebin, cudaMemcpyHostToDevice);
  ...
  // the grid.  Here, we are using a 2D grid, where the total of nblock *
  // nthreads in each dimension is natoms (or the closest multiple bigger than
  // natoms).
  ...
  // Note: Function is now called with device variables!
  pair_gpu<<<nblock, nthreads>>>(d_x, d_y, d_z, d_g, numatm, nconf, xbox, ybox,
                                 zbox, box, del);
  nvtxRangePop();
  ...
  cudaMemcpy(h_g, d_g, sizebin, cudaMemcpyDeviceToHost);
  //////////////////////////////////////////////////////////////////////////////
